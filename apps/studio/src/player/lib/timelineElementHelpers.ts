...
/**
 * Elements hidden from the Studio timeline only. Companion audio must remain
 * in the preview DOM for HyperFrames playback, but it is an implementation
 * detail of an A-roll video and should not appear as a second editable track.
 */
export function isStudioTimelineHiddenElement(el: Element): boolean {
  return (
    isTimelineIgnoredElement(el) ||
    Boolean(el.closest("[data-workbench-companion-audio]"))
  );
}
...
export function readTimelineDurationFromDocument(doc: Document | null | undefined): number {
  ...
  for (const node of Array.from(doc.querySelectorAll("[data-start]"))) {
    if (isStudioTimelineHiddenElement(node)) continue;
...
function getTimelineDomNodes(doc: Document): Element[] {
  ...
  return Array.from(doc.querySelectorAll("[data-start]")).filter(
    (node) => node !== rootComp && !isStudioTimelineHiddenElement(node),
  );
}
...
export function isImplicitTimelineLayerCandidate(root: Element, el: Element): el is HTMLElement {
  ...
  if (isStudioTimelineHiddenElement(el)) return false;
