...
describe("usePlayerStore", () => {
  ...
  describe("requestSeek", () => {
    it("records whether an external seek should keep playback running", () => {
      usePlayerStore.getState().requestSeek(4.2, { keepPlaying: true });

      expect(usePlayerStore.getState().requestedSeekTime).toBe(4.2);
      expect(usePlayerStore.getState().requestedSeekKeepPlaying).toBe(true);

      usePlayerStore.getState().clearSeekRequest();
      expect(usePlayerStore.getState().requestedSeekTime).toBeNull();
      expect(usePlayerStore.getState().requestedSeekKeepPlaying).toBe(false);
    });
  });
