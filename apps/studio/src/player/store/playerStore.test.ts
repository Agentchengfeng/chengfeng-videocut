...
describe("usePlayerStore", () => {
  ...
  describe("requestSeek", () => {
    it("carries keepPlaying through the external seek request", () => {
      usePlayerStore.getState().requestSeek(7.5, { keepPlaying: true });

      expect(usePlayerStore.getState()).toMatchObject({
        requestedSeekTime: 7.5,
        requestedSeekOptions: { keepPlaying: true },
      });

      usePlayerStore.getState().clearSeekRequest();
      expect(usePlayerStore.getState().requestedSeekTime).toBeNull();
      expect(usePlayerStore.getState().requestedSeekOptions).toBeNull();
    });
  });
