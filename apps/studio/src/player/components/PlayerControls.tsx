...
// Both glyphs share a centered 24px coordinate system so the morph does not
// shift the icon's optical center when playback toggles.
const PLAY_BLADE_D = "M8 5L19 12L8 19Z";
const PAUSE_BARS_D = "M6.5 5H10.5V19H6.5Z M13.5 5H17.5V19H13.5Z";
  ...
  return (
    <div
      ...
      style={{
        ...
        width: "2px",
        height: "10px",
        background: "var(--hf-accent, #D97757)",
        borderRadius: "1px",
      }}
    />
  );
...
function WorkAreaOverlay({
  ...
  return (
    <>
      ...
      />
      {inPoint !== null && (
        <SeekBarMarker position={inPoint} duration={duration} />
      )}
      {outPoint !== null && (
        <SeekBarMarker position={outPoint} duration={duration} />
      )}
    </>
  );
}
...
const SeekBar = memo(function SeekBar({
  ...
  return (
    <div
      ref={(el) => {
        (seekBarRef as React.MutableRefObject<HTMLDivElement | null>).current =
          el;
    ...
    >
      <div
        ...
        style={{ background: "rgba(38,38,37,0.14)", height: "3px" }}
      >
        <WorkAreaOverlay
          inPoint={inPoint}
          outPoint={outPoint}
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, var(--hf-accent, #D97757), #B95539)",
          }}
        />
        <div
          ...
          style={{
            background: "var(--hf-accent, #D97757)",
            boxShadow:
              "0 0 0 3px rgba(217,119,87,0.14), 0 1px 4px rgba(38,38,37,0.18)",
          }}
        />
      </div>
    </div>
...
export const PlayerControls = memo(function PlayerControls({
  ...
  return (
    <div
      className="cf-player-control-row px-4 py-2 flex flex-wrap items-center gap-x-2 gap-y-1"
      aria-disabled={disabled || undefined}
      style={{
        borderTop: "1px solid var(--cf-line, #E5E1D8)",
    ...
    >
      <Tooltip label={isPlaying ? "暂停" : "播放"}>
        <button
          ...
          className="cf-play-button flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-md disabled:opacity-30 disabled:pointer-events-none transition-colors"
          style={{ background: "var(--hf-accent, #D97757)" }}
        >
      ...
      >
        <button
          ...
          style={{ color: "var(--cf-muted, #6E6B64)", cursor: "pointer" }}
        >
          ...
          {timeDisplayMode === "time" ? (
            <>
              <span
                style={{
                  color: "var(--cf-line-strong, #D9D4C5)",
                  margin: "0 2px",
                }}
              >
                /
              </span>
              <span style={{ color: "var(--cf-faint, #9C988E)" }}>
                {formatTime(duration)}
              </span>
            </>
          ) : null}
