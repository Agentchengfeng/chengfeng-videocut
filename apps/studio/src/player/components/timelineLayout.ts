...
function getMajorTickInterval(
  ...
  if (Number.isFinite(pixelsPerSecond) && (pixelsPerSecond ?? 0) > 0) {
    ...
    return (
      zoomIntervals.find(
        (interval) => interval * (pixelsPerSecond ?? 0) >= targetMajorPx,
      ) ?? 600
    );
  }
...
export function generateTicks(
  ...
  for (
    let t = 0;
    t <= duration + 0.001 && major.length < maxTicks;
    t += majorInterval
  ) {
...
export function generateVisibleTicks(
  ...
  const visible =
    viewportWidth > gutter ? (viewportWidth - gutter) / pixelsPerSecond : 0;
...
export function getTimelineScrollLeftForZoomAnchor(input: {
  ...
  const timelineX = Math.max(
    0,
    input.currentScrollLeft + input.pointerX - input.gutter,
  );
  const timeAtPointer = Math.max(
    0,
    Math.min(input.duration, timelineX / currentPps),
  );
...
/* ── Asset drop ───────────────────────────────────────────────────── */
export function getDefaultDroppedTrack(
  trackOrder: number[],
  rowIndex?: number,
): number {
...
export function resolveTimelineAssetDrop(
  ...
  const start = Math.max(
    ...
    Math.min(
      input.duration,
      Math.round((x / Math.max(input.pixelsPerSecond, 1)) * 100) / 100,
    ),
  );
