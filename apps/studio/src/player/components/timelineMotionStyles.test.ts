...
describe("timeline motion styles", () => {
  ...
  it("layers the active coral bloom through opacity instead of a gradient background swap", () => {
    ...
    expect(baseTimelineClipRule).toContain("background-color: #ffffff");
    expect(activeTimelineClipRule).not.toContain("background: linear-gradient");
    expect(activeTimelineClipRule).toContain("border-color: rgba(217, 119, 87, 0.72)");
    expect(activeTimelineClipRule).not.toContain("box-shadow");
    expect(bloomOverlayRule).toContain("background: rgba(217, 119, 87, 0.12)");
