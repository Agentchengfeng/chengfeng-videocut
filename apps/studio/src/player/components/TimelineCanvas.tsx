...
function ClipLintDot({ element }: { element: TimelineElement }) {
  const lint = usePlayerStore((s) =>
    s.lintFindingsByElement.get(element.key ?? element.id),
  );
...
export const TimelineCanvas = memo(function TimelineCanvas({
  ...
  const hasCustomClipContent = (element: TimelineElement) =>
    Boolean(renderClipContent) &&
    !(
      element.tag === "div" &&
      (element.timelineRole === "a-roll" || element.timelineRole === "captions")
    );
  const renderClipChildren = (
    element: TimelineElement,
    clipStyle: TrackVisualStyle,
  ) => {
    const hasCustomContent = hasCustomClipContent(element);
    return (
      <>
        {renderClipOverlay?.(element)}
        {!hasCustomContent && <ClipLintDot element={element} />}
  ...
  const activeDropPlacement =
    draggedClip?.started === true
      ? (draggedClip.previewStackingReorder?.placement ?? null)
      : null;
    ...
    >
...

      {
        ...
        displayTrackOrder.map((layerId, rowIndex) => {
          ...
          const isFirstAudioLayer =
            isAudioLayer && previousLayer?.kind !== "audio";
          const isPendingTrack =
            draggedClip?.started === true &&
            !trackOrder.includes(layerId) &&
            els.length === 0;
          const baseRowBackground =
            rowIndex % 2 === 0 ? theme.rowBackground : "#0D0E12";
          ...
          const isTrackHidden =
            els.length > 0 && els.every((element) => element.hidden === true);
          return (
            <Fragment key={layerId}>
              ...
              <div
                ...
                style={{
                  ...
                  borderTop: isFirstAudioLayer
                    ? `2px solid ${theme.rulerBorder}`
                    : undefined,
                  borderBottom: `1px solid ${theme.rowBorder}`,
                  boxShadow:
                    dropIndicator?.kind === "onto"
                      ? `inset 0 0 0 1px ${ts.accent}`
                      : undefined,
                }}
              >
                <TimelineLayerGutter
                  ...
                  rowTrack={rowTrack}
                  timelineRole={
                    els.find((element) => element.timelineRole)?.timelineRole
                  }
                  theme={theme}
                  onToggleHidden={() => {
                    if (onToggleElementHidden && els.length > 0) {
                      for (const element of els) {
                        void onToggleElementHidden(
                    ...
                    els.map((el) => {
                      ...
                      return (
                        <TimelineClip
                          ...
                          isDragging={false}
                          hasCustomContent={hasCustomClipContent(
                            previewElement,
                          )}
                          ...
                          onResizeStart={(edge, e) => {
                            if (
                              ...
                              return;
                            if (edge === "start" && !capabilities.canTrimStart)
                              return;
                            if (edge === "end" && !capabilities.canTrimEnd)
                              return;
                            e.stopPropagation();
                          ...
                          onPointerDown={
                            ...
                            (e) => {
                              ...
                              if (!onMoveElement || !capabilities.canMove)
                                return;
                              blockedClipRef.current = null;
                              setShowPopover(false);
                              setRangeSelection(null);
                          ...
                          onClick={(e) => {
                            ...
                            if (activeTool === "razor" && onRazorSplit) {
                              ...
                              const splitTime =
                                previewElement.start + clickOffsetX / pps;
                          ...
                          {STUDIO_KEYFRAMES_ENABLED &&
                            keyframeCache?.get(elementKey) && (
                              <TimelineClipDiamonds
                                ...
                                onClickKeyframe={(pct) =>
                                  onClickKeyframe?.(previewElement, pct)
                                }
      ...
      {activeDraggedElement && activeDraggedPosition && (
        <TimelineDragGhost
          ...
          hasCustomContent={hasCustomClipContent(activeDraggedElement)}
          theme={theme}
        >
          {renderClipChildren(
            activeDraggedElement,
            getTrackStyle(activeDraggedElement.tag),
          )}
        </TimelineDragGhost>
      )}
