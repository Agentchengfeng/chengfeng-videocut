...
function getInitialRightInspectorPanes(
...
export function usePanelLayout(initialState?: InitialPanelLayoutState) {
  ...
  const [rightCollapsed, setRightCollapsed] = useState(
    initialState?.rightCollapsed ?? false,
  );
  const [rightPanelTab, setRightPanelTab] = useState<RightPanelTab>(
    initialState?.rightPanelTab ?? "design",
  );
  const [rightInspectorPanes, setRightInspectorPanes] =
    useState<RightInspectorPanes>(() =>
