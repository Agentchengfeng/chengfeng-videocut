...
export interface TimelineMoveGestureContext {
  /** Stable key of the clip the user physically grabbed. */
  primaryElementKey: string;
  /** Whether the grabbed clip belonged to a real multi-selection. */
...
export function persistTimelineMoveEditsAtomically(
  ...
  return deps.handleTimelineGroupMove(
    ...
    { coalesceKey, coalesceMs, operation, ...gesture },
  );
}
