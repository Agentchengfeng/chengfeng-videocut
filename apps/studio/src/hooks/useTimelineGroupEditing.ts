...
export interface TimelineGroupCommitOptions {
  ...
  coalesceMs?: number;
  /** Native drag producer intent. Embedding adapters use this to distinguish
   *  one grabbed clip plus track-topology companions from a real batch edit. */
  operation?: "timing" | "lane-reorder" | "track-insert";
  /** Stable key of the clip the user physically grabbed for this gesture. */
  primaryElementKey?: string;
  /** True only when the gesture started from an actual multi-selection. */
