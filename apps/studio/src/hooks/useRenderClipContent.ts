...
export function useRenderClipContent({
  ...
  return useCallback(
    ...
    (el: TimelineElement, style: { clip: string; label: string }): ReactNode => {
      ...
      // EDL A-roll segments are lightweight timeline metadata backed by one
      // shared video. Per-segment composition captures create a request storm.
      if (el.edlSegmentId) return null;
