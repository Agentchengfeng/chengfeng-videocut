...
function useKeyframeToggle(session?: DomEditSessionSlice) {
  ...
  if (kfAnim?.keyframes && sel) {
    if (!isPlayheadWithinTween(kfAnim, currentTime)) {
      ...
      state = kfAnim.keyframes.keyframes.some(
...
export function TimelineToolbar({
  ...
  return (
    <div className="cf-timeline-toolbar border-b border-neutral-800/40 bg-neutral-950/96">
        ...
        <div className="flex items-center gap-3">
          ...
          {STUDIO_RAZOR_TOOL_ENABLED && (
            <div className="flex items-center border border-neutral-800 rounded overflow-hidden">
              ...
              <Tooltip label="切分工具 (B)，按住 Shift 可切分全部轨道">
                <button
                  type="button"
          ...
          {onSplitElement &&
            (() => {
              ...
              const { selectedElementId, elements, currentTime } =
          ...
          {beatAnalysisReady &&
            (() => {
              ...
              return (
                <Tooltip
                  label={canAdd ? "在播放头添加节拍" : "播放头位置已有节拍"}
                >
                  <button
                    type="button"
                    disabled={!canAdd}
