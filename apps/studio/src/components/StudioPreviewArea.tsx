...
import { WorkbenchTaskPanel } from "./WorkbenchTaskPanel";

const CANVAS_ASSIST_TOOLBAR_VISIBLE = false;
  ...
  const resolveKeyframeTarget = useCallback(
    ...
    (pct: number): { animId: string; tweenPct: number } | null => {
      const cached = usePlayerStore
        .getState()
        .keyframeCache.get(domEditSelection?.id ?? "");
      const kf = cached?.keyframes.find(
        (k) => Math.abs(k.percentage - pct) < 0.2,
      );
      const group = kf?.propertyGroup;
      const anim =
        (group
          ? selectedGsapAnimations.find((a) => a.propertyGroup === group)
          : undefined) ?? selectedGsapAnimations.find((a) => a.keyframes);
      return anim
        ? { animId: anim.id, tweenPct: kf?.tweenPercentage ?? pct }
        : null;
    },
  ...
  const timelineEditCallbacks = useMemo(
    () => ({
      ...
      onMoveKeyframeToPlayhead: (_elId: string, pct: number) => {
        const target = resolveKeyframeTarget(pct);
        if (target)
          handleGsapMoveKeyframeToPlayhead(target.animId, target.tweenPct);
      },
        ...
        const pct =
          el.duration > 0
            ? Math.max(
                ...
                Math.min(
                  100,
                  Math.round(((currentTime - el.start) / el.duration) * 100),
                ),
              )
            : 0;
        const anim = selectedGsapAnimations.find((a) => a.keyframes);
    ...
    <div className="flex-1 flex flex-col relative min-w-0">
      <div className="flex-1 min-h-0 relative">
        <TimelineEditProvider value={timelineEditCallbacks}>
          <NLELayout
            projectId={projectId}
            taskPanel={<WorkbenchTaskPanel key={projectId} projectId={projectId} />}
              ...
              blockPreview ? (
                ...
                <>
                  <DomEditOverlay
                    ...
                    selection={
                      shouldShowSelectedDomBounds ? domEditSelection : null
                    }
                    groupSelections={
                      shouldShowSelectedDomBounds ? domEditGroupSelections : []
                    }
                    allowCanvasMovement={
                      STUDIO_PREVIEW_MANUAL_EDITING_ENABLED &&
                      !isGestureRecording
