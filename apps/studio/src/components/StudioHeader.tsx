...
export interface StudioHeaderProps {
  ...
  onExport?: () => void;
  viewOptions?: ReadonlyArray<{ mode: StudioViewMode; label: string }>;
}
...
const VIEW_MODE_OPTIONS: Array<{ mode: StudioViewMode; label: string }> = [
  { mode: "storyboard", label: "分镜" },
  { mode: "timeline", label: "预览" },
];
...
function ViewModeToggle({
  ...
  extensionOptions?: ReadonlyArray<{ mode: StudioViewMode; label: string }>;
}) {
  const { viewMode, setViewMode } = useViewMode();
  const tabRefs = useRef<Array<HTMLButtonElement | null>>([]);
  const options = [...VIEW_MODE_OPTIONS, ...extensionOptions];
...
export function StudioHeader({
  ...
  return (
    <div className="flex items-center justify-between h-10 px-3 bg-neutral-900 border-b border-neutral-800 flex-shrink-0">
      ...
      <ViewModeToggle extensionOptions={viewOptions} />
