...
interface NLELayoutProps {
  projectId: string;
  portrait?: boolean;
  /** Task-specific workbench panel shown to the right of the persistent preview. */
  taskPanel?: ReactNode;
...
export function shouldDisableTimelineWhileCompositionLoading(
...
export const NLELayout = memo(function NLELayout({
      ...
      if (!resolvedPath) {
        try {
          ...
          if (doc) {
            ...
            if (host) {
              resolvedPath =
                host.getAttribute("data-composition-src") || undefined;
            }
          }
        } catch {
          /* cross-origin */
  ...
  const { onMoveElement, onResizeElement, onSplitElement } =
  ...
  const handleSplitElement = useCallback(
    (element: TimelineElement, splitTime: number) => {
      ...
      return onSplitElement?.(
        toLocalElement(element, basis),
        Math.max(0, splitTime - basis),
      );
    },
  ...
  useMountEffect(() => {
    fetch(`/api/projects/${projectId}/files/index.html`)
      ...
      .catch((err: unknown) => {
        ...
        console.warn(
          "[studio] Couldn't load composition source map from index.html:",
          err,
        );
      });
  });
  ...
  useEffect(() => {
    ...
    const patchElements = (
      ...
      const updated = elements.map((el) => {
        ...
        const src =
          map.get(el.id) ?? map.get(el.id.replace(/-(host|comp|layer)$/, ""));
        if (src) {
          patched = true;
    ...
    return usePlayerStore.subscribe((state, prev) => {
      if (patching) return;
      if (state.elements === prev.elements || state.elements.length === 0)
        return;
  ...
  const [timelineH, setTimelineH] = useState(() => {
    ...
    return stored !== undefined && stored >= MIN_TIMELINE_H
      ? Math.max(stored, DEFAULT_TIMELINE_H)
      : DEFAULT_TIMELINE_H;
  });
  ...
  const directUrl =
    compositionStack.length > 1 ? currentLevel.previewUrl : undefined;
  ...
  return (
    <div
      ref={containerRef}
      className="cf-nle-layout flex flex-col h-full min-h-0 bg-neutral-950"
      onKeyDown={handleKeyDown}
      tabIndex={-1}
      data-studio-fullscreen-target=""
    >
      {/* Persistent preview on the left, task functions on the right. */}
      <div className="cf-workbench-upper flex-1 min-h-0 flex">
        <div className="cf-workbench-viewer flex-1 min-w-0 min-h-0 flex flex-col">
          <div
          ...
          <div className="cf-player-controls bg-neutral-950 border-t border-neutral-800/50 flex-shrink-0">
            ...
            <PlayerControls
              ...
              showSeekBar={!isTimelineVisible || isFullscreen}
            />
          </div>
        </div>
        {!isFullscreen && taskPanel}
      </div>
      ...
      {!isFullscreen && isTimelineVisible ? (
        <>
          ...
          {/* Timeline section */}
          <div
            className="cf-timeline-section relative flex flex-col flex-shrink-0"
          ...
          >
            ...
            {timelineFooter && (
              <div className="flex-shrink-0">{timelineFooter}</div>
            )}
            {timelineDisabled && (
              <div
