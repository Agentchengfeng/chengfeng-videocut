...
import { installPreviewEdlAudioCleanup } from "../lib/timelineIframeHelpers";
...
function mediaBlocksAssetReadiness(el: HTMLMediaElement): boolean {
  if (el.preload === "none") return false;

  const requiredReadyState =
    el.preload === "metadata" ? MEDIA_HAVE_METADATA : MEDIA_HAVE_FUTURE_DATA;
  return el.readyState < requiredReadyState;
}
  ...
  (
    ...
    useMountEffect(() => {
      ...
      import("@hyperframes/player").then(() => {
        ...
        const cleanupEdlAudio = installPreviewEdlAudioCleanup(iframe, player);
