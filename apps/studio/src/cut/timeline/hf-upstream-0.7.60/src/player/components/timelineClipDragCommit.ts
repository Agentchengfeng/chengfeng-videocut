...
export interface DragCommitDeps {
  ...
  onMoveElements?: (
    ...
    coalesceMs?: number,
    gesture?: TimelineMoveGestureContext,
  ) => Promise<void> | void;
  /**
...
export function persistMoveEdits(
  ...
  const persisted = onMoveElements
    ? onMoveElements(persistEdits, coalesceKey, operation, coalesceMs, gesture)
...
export function commitDraggedClipMove(drag: DraggedClipState, deps: DragCommitDeps): void {
  ...
  const gesture: TimelineMoveGestureContext = {
    primaryElementKey: dragKey,
    multiSelection: multi !== null,
  };
  ...
  if (!isInsert && !laneChanged) {
    ...
    if (multi) {
      ...
      void persistMoveEdits(edits, deps, undefined, "timing", undefined, gesture);
      return;
    }
  ...
  if (!isVertical || !deps.readZIndex || !deps.onStackingPatches) {
    void persistMoveEdits(edits, deps, coalesceKey, "lane-reorder", undefined, gesture);
    return;
  }
  void runLaneZGesture({
    commitLane: () =>
      persistMoveEdits(edits, deps, coalesceKey, "lane-reorder", undefined, gesture),
    commitZ: () =>
      syncStackingForEdit(
        candidate,
        dragKey,
...
function commitTrackInsert(
  ...
  if (!deps.readZIndex || !deps.onStackingPatches) {
    void persistMoveEdits(edits, deps, coalesceKey, "track-insert", undefined, gesture);
    return;
  }
  void runLaneZGesture({
    commitLane: () =>
      persistMoveEdits(edits, deps, coalesceKey, "track-insert", undefined, gesture),
    commitZ: () =>
