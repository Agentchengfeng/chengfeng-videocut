/**
 * Forked from HyperFrames Studio 0.7.60
 * `apps/studio/src/player/components/timelineLayout.ts`.
 *
 * Product reduction: only the Store-owned `ZoomMode` type and time formatter
 * are replaced. Geometry, ticks, fit headroom, extent and playhead math remain
 * source-derived.
 */
 ...
 * This is physical canvas space only: Product must not expose it as additional
 * composition time in the ruler, Player, or screen-reader summary.
 */
...
/**
 * Fit-mode pixels-per-second: fill the viewport with the true composition
 * duration, while never mapping fewer than MIN_TIMELINE_EXTENT_S physical
 * seconds onto it — a short comp takes a fraction of the width and keeps an
 * empty drag/drop surface on the right. The surface is deliberately not an
 * extension of the user-visible time domain.
 ...
 * RENDERED extent grows; clip positions/durations and the ruler's semantic
