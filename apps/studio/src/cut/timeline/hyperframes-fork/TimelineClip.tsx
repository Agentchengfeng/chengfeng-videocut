...
import {
  ...
  type ForkTimelineElement,
  type TimelineTheme,
} from "./timelineTheme";

/** Forked from HyperFrames Studio 0.7.60 TimelineClip.tsx. */
export interface ForkTimelineEditCapabilities {
  canTrimStart: boolean;
  canTrimEnd: boolean;
}

export interface TimelineClipProps {
  el: ForkTimelineElement;
  pps: number;
  clipY: number;
  isSelected: boolean;
  isHovered: boolean;
  ...
  capabilities: ForkTimelineEditCapabilities;
  theme?: TimelineTheme;
...
export const TimelineClip = memo(function TimelineClip({
  ...
  return (
    <div
      ...
      tabIndex={tabIndex ?? (role === "option" ? 0 : undefined)}
      title={
        isComposition
    ...
    >
      ...
      {showHandles && capabilities.canTrimStart && (
        <div
          className="cf-cut-trim-handle is-start"
          aria-hidden="true"
      ...
      {showHandles && capabilities.canTrimEnd && (
        <div
          className="cf-cut-trim-handle is-end"
          aria-hidden="true"
