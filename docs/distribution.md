...
由同一个版本的 GitHub Release / CI 产物承载。**v0.4.10 是 Windows Desktop 受控测试
...
v0.4.10 Windows Desktop 受控测试 Release 应提供：
...
macOS DMG 不在 v0.4.10 附件中；未来单独提供 macOS Desktop 测试时，必须另列资产与
...
v0.4.10 预发布测试使用固定 tag 的安装命令（不是可移动的 `main` 或
`releases/latest`）：
...
8. v0.4.10 的 Windows 桌面资产必须额外通过无提权 NSIS 安装、App 父进程退出后服务
