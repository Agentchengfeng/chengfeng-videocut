...
`chengfeng-videocut` 主仓维护 Runtime、Studio、CLI 与总体安装入口。新安装从 [INSTALL](../INSTALL.md) 开始；公开六个独立 Skill 仓库及各自的安装说明也由该入口导航，本页不复制版本清单。

六个独立包各自维护本项方法、必要资源与依赖说明。其中 `chengfeng-videocut-install` 是安装方法的唯一真源，主 Plugin 引用其同源快照，不另维护第二套安装方法。旧 `chengfeng-videocut-skills` 仓库只提供迁移导航并保留旧代码与历史，不再作为新安装实现的维护入口。

主 Plugin 的新职责是安装引导与组合清单，不批量内置业务 Skills；旧市场入口不能视为新组合的安装入口。当前安装方法不要求 MCP，不因安装某个 Skill 自动安装 Runtime 或执行业务任务。

## 已公开范围与未完成项

六包公开的 `v0.1.0-beta.1` 是文件级源码预览：包文件安装、宿主加载、Runtime 能力和真实业务结果分别验收。单包安装不代表已经取得完整工作台或整套能力。
...
以下原文完整保留，只供辨认旧版本、旧实现与历史发布约束。其入口、命令、包边界及“当前”状态均属于当时语境，不能用于新入口，也不表示对应发行已经通过验收。

<details>
<summary>v0.4.9 历史发布契约原文（不适用于新入口）</summary>
