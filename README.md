...
0.5.2 的目标入口是 Codex Plugin。Plugin 调用平台原生 installer，用户机器不需要预装
...
当前 0.5.2 代码与本机隔离安装已实现，但**公开发布被阻止**：FFmpeg/FFprobe/Bun/
...
`install.cjs` 和 Desktop/Companion 只保留历史兼容或实验用途，不是 0.5.2 用户入口。
