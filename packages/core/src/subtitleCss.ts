/**
 * How a stored subtitle style becomes the CSS that draws it.
 *
 * This lives in core rather than beside the preview because two renderers now
 * draw the same document: the workbench overlay, and the export. A second
 * implementation that agrees by inspection is a drift waiting to happen — the
 * preset swatch and the frame overlay already drifted once this way, one
 * putting a 3px radius on the plate while the other put 0.1em on the same
 * style, so the look you picked was not the look you were shown.
 *
 * It returns a declaration object rather than a string so the browser can hand
 * it straight to React and the exporter can serialise it, and it takes the
 * font size as a unit-bearing string so the caller decides what one percent
 * means: `cqh` against the picture in the preview, `px` at the output size in
 * an export. Same numbers, same shape, two sizes.
 */

import type { SubtitleStyle } from "./subtitles";

export type StyleDeclarations = Record<string, string | number>;
 ...
 * filled in the counters of 口 and 日 from the inside.
 */
 ...
 * tracking. So the caller decides what one percent means and the same numbers
 * produce the same shape at both sizes.
 */
...
/** The same declarations as a CSS text block, for a renderer that writes HTML. */
export function declarationsToCssText(declarations: StyleDeclarations): string {
